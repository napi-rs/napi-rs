//! napi adapter: registers the shared scheduler as the addon's
//! [`AsyncRuntime`] backend and exports the JavaScript-facing host protocol
//! (CurrentThread task host, JS timer host + relay, config/metrics).
//!
//! Hosts call [`install`] from their own `#[napi_derive::module_init]` hook.
//! This module deliberately ships no `module_init` of its own: the host must
//! resolve its runtime configuration (environment variables, defaults) first
//! and stay in charge of that resolution order.

use std::{future::Future, pin::Pin, ptr};

use napi::bindgen_prelude::{
  AsyncRuntime, AsyncRuntimeRejection, AsyncRuntimeTask, FnArgs, Promise, Unknown,
...
struct SharedAsyncRuntime;

// SAFETY: See the implementation notes in this crate's README. Shutdown closes
...
unsafe impl AsyncRuntime for SharedAsyncRuntime {
  fn spawn(
    &self,
    ...
    match try_spawn(task) {
      ...
      Err((error, task)) => Err(AsyncRuntimeRejection::new(
        task,
  ...
  fn spawn_blocking(
    ...
    // as the scheduler's spawn_blocking facade. See the implementation notes in this crate's README.
    match try_spawn_blocking(work) {
      Ok(handle) => {
        handle.detach();
        Ok(())
      }
      Err((error, work)) => Err(AsyncRuntimeRejection::new(
        work,
        napi::Error::from_reason(error.to_string()),
      )),
    }
...
///
/// Reports the runtime controller's validated options, including a
...
fn registered_current_thread_task_host(
  ...
  let mut registrations = NATIVE_CURRENT_THREAD_TASK_HOSTS
    .lock()
    .unwrap_or_else(std::sync::PoisonError::into_inner);
...
impl NativeCurrentThreadTaskHostInner {
  fn new(env: &napi::Env, host_registration: u64) -> napi::Result<std::sync::Arc<Self>> {
    const ASYNC_RESOURCE_NAME: &[u8] = b"napi_async_runtime_current_thread_task_host";
    ...
    let finalize_data = std::sync::Weak::into_raw(std::sync::Arc::downgrade(&inner))
      .cast_mut()
      .cast();
    ...
    *inner
      .threadsafe_function
      .lock()
      .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(threadsafe_function as usize);
  ...
  fn dispatch(&self, delivery: CurrentThreadTaskDelivery) -> bool {
    ...
    let Some(status) =
      ...
      unsafe {
        drop(Box::<NativeCurrentThreadTaskHostPayload>::from_raw(
          data.cast(),
        ));
      }
      return false;
...
pub fn register_current_thread_task_host(
  ...
  let inner = NativeCurrentThreadTaskHostInner::new(env, host_registration)?;
  {
    let mut slot = inner
      ...
      .lock()
      .unwrap_or_else(std::sync::PoisonError::into_inner);
    *slot = Some(register_current_thread_task_driver(std::sync::Arc::new(
      NativeCurrentThreadTaskHost {
...
impl HostTimerRelayHealth {
  fn set_cancellation_accounting(&self, accounting: RelayCancellationAccounting) {
    self
      ...
      .store(accounting as u8, std::sync::atomic::Ordering::Release);
  }
...
fn registered_timer_host(id: u64) -> Option<std::sync::Arc<JsTimerHostInner>> {
  let mut registrations = JS_TIMER_HOSTS
    .lock()
    .unwrap_or_else(std::sync::PoisonError::into_inner);
  let inner = registrations.get(&id).and_then(std::sync::Weak::upgrade);
  if inner.is_none() {
...
fn register_pending_host_timer_if_live(
  ...
  Ok(register_pending_host_timer_locked(
    &mut pending,
    relay_ids,
    id,
    waker,
...
fn take_pending_host_timer(
  ...
  if pending
    .get(&id)
    .is_some_and(|slot| slot.relay_id == relay_id)
  {
    pending
      .get(&id)
...
impl JsTimerHostInner {
  ...
  fn handle_cancellation_failure(
    ...
    let operation = if queue_failure {
      "could not be queued"
    } else {
      "failed"
    };
    if relay_health.cancellation_accounting() == RelayCancellationAccounting::CleanupOnly {
      recover_host_timer_failure(
        || {},
        format_args!(
          "napi-async-runtime: host timer cancellation callback {operation} for relay {relay_id} during \
           cleanup: {error}"
        ),
      );
      return;
    ...
    match record_host_timer_failure(
      ...
      HostTimerFailureAction::Duplicate => {
        recover_host_timer_failure(
          || {},
          format_args!(
            "napi-async-runtime: host timer cancellation callback {operation} for relay {relay_id} after \
      ...
      HostTimerFailureAction::EvictHost => {
        recover_host_timer_failure(
          || self.evict(),
          format_args!(
            "napi-async-runtime: host timer cancellation callback {operation} for relay {relay_id} (host \
             gone, evicting): {error}"
          ),
        );
      }
      HostTimerFailureAction::EvictHostAfterStrikes(strikes) => {
        recover_host_timer_failure(
          || self.evict(),
          format_args!(
            "napi-async-runtime: host timer cancellation callback {operation} {strikes} times in a row, \
      ...
      HostTimerFailureAction::Retry(strikes) => {
        recover_host_timer_failure(
          || {},
          format_args!(
            "napi-async-runtime: host timer cancellation callback {operation} for relay {relay_id} \
  ...
  /// live registrant left the re-poll fails LOUD in the scheduler).
...
impl TimerDriver for JsTimerHost {
  fn register(&self, id: TimerId, deadline: std::time::Instant, waker: std::task::Waker) {
    ...
    let (relay_id, cancellation, relay_health) = match self.inner.register_pending(id, waker) {
      ...
      Ok(PendingHostTimerRegistration::Armed {
        relay_id,
        cancellation,
        relay_health,
      ...
      Ok(PendingHostTimerRegistration::Exhausted(waker)) => {
        recover_host_timer_failure(
          ...
          format_args!(
            "napi-async-runtime: host timer relay id space exhausted, evicting this timer host"
          ),
        );
        return;
      }
          ...
          match action {
            HostTimerFailureAction::EvictHost => {
              recover_host_timer_failure(
                || inner.evict(),
                format_args!(
                  "napi-async-runtime: host timer callback failed (host gone, evicting): {error}"
                ),
              );
            }
            HostTimerFailureAction::EvictHostAfterStrikes(strikes) => {
              recover_host_timer_failure(
                || inner.evict(),
                format_args!(
                  "napi-async-runtime: host timer callback failed {strikes} times in a row, evicting this \
                   timer host: {error}"
                ),
              );
            }
            HostTimerFailureAction::Retry(strikes) => {
              recover_host_timer_failure(
                ...
                format_args!(
                  "napi-async-runtime: host timer callback failed \
                   ({strikes}/{HOST_TIMER_MAX_TRANSIENT_FAILURES} before eviction): {error}"
                ),
              );
            }
            HostTimerFailureAction::Duplicate => {
              recover_host_timer_failure(
                ...
                format_args!(
                  "napi-async-runtime: host timer callback failed after this relay failure was already \
                   accounted: {error}"
                ),
              );
            }
  ...
  fn cancel(&self, id: TimerId) {
    let pending = {
      ...
      if let Some(timer) = pending.get(&id) {
        timer
          .relay_health
          .set_cancellation_accounting(RelayCancellationAccounting::HostHealth);
      }
      pending.remove(&id)
    };
...
/// CurrentThread timers. Called at import by every
...
/// non-numeric, OR zero value as "unset" and fall back to `default`, then
/// clamp the result to `maximum`. A `0` must never survive here because the
/// host's module-init path feeds this count into a constructor that rejects
/// `0`: [`RuntimeOptions`] validation rejects a `0` thread count, which would
/// panic an `expect()` during addon load over what may be a simple typo in an
...
mod tests {
  use super::{
    BindingHostRegistration, BindingRuntimeFlavor, BindingRuntimeOptions,
    HOST_TIMER_MAX_TRANSIENT_FAILURES, HostTimerFailureAction, HostTimerRelayHealth,
    MAX_SAFE_JS_INTEGER, NATIVE_TASK_HOST_AFTER_DRIVE_TEST_HOOK,
    NATIVE_TASK_HOST_AFTER_PAYLOAD_DROP_TEST_HOOK, NativeCurrentThreadTaskHostInner,
    NativeCurrentThreadTaskHostPayload, PendingHostTimer, PendingHostTimerRegistration,
    PendingRelayDropGuard, PendingRelayState, RelayCancellationAccounting, RelayIdAllocator,
    RelayScheduleState, SharedAsyncRuntime, call_native_current_thread_task_host,
    claim_host_registration_id, complete_relay_schedule_callback,
    get_current_thread_task_host_contract_version, install_cleanup_hook_or_rollback,
    ...
    reserve_current_thread_host_registration, reset_host_timer_failures_after_success,
    resolve_thread_count, retire_pending_relay, retire_pending_relays, safe_js_number,
    take_pending_host_timers, unregister_current_thread_task_host, wake_host_timer_safely,
  };

  #[derive(Default)]
  struct TestPendingRelayState {
    pending:
      std::sync::Mutex<rustc_hash::FxHashMap<crate::async_runtime::TimerId, PendingHostTimer>>,
    relay_ids: RelayIdAllocator,
  ...
  impl PendingRelayState for TestPendingRelayState {
    fn take_pending_relay(
      &self,
      id: crate::async_runtime::TimerId,
      relay_id: u32,
      accounting: RelayCancellationAccounting,
    ) -> Option<PendingHostTimer> {
  ...
  fn test_pending_relay_guard(
    ...
    u32,
    std::sync::Arc<HostTimerRelayHealth>,
    PendingRelayDropGuard<TestPendingRelayState>,
  ) {
    let relay_id = state
      .relay_ids
      .reserve()
      .expect("the test relay id must be available");
    let relay_health = std::sync::Arc::new(HostTimerRelayHealth::default());
    let replaced = state
      .pending
      .lock()
    ...
    (
      relay_id,
      relay_health,
      PendingRelayDropGuard::new(std::sync::Arc::clone(state), id, relay_id),
    )
  }

  struct BudgetBoundaryTimerWait {
    state: std::sync::Arc<TestPendingRelayState>,
    timer_id: crate::async_runtime::TimerId,
    schedule_state: RelayScheduleState,
    registered: bool,
  }
  ...
  impl std::future::Future for BudgetBoundaryTimerWait {
    ...
    fn poll(
      ...
      assert!(
        !self.registered,
        "the timer wait must be cancelled before a second poll"
      );
      ...
      crate::async_runtime::spawn_detached(async move {
        let mut relay_drop_guard = relay_drop_guard;
  ...
  fn host_registration_capability_round_trips_without_precision_loss() {
    for id in [1, u64::from(u32::MAX), u64::from(u32::MAX) + 1, u64::MAX] {
      ...
      assert_eq!(
        BindingHostRegistration::id(registration.high, registration.low),
        id
      );
    }
  }
  ...
  fn binding_runtime_options_convert_to_a_partial_core_patch() {
    use crate::async_runtime::{RuntimeFlavor, RuntimeOptionsPatch};
  ...
  fn binding_runtime_options_reject_unsafe_thread_counts() {
    use crate::async_runtime::RuntimeOptionsPatch;
    ...
    for value in [
      ...
      assert!(
        error
          .reason
          .contains("`workerThreads` must be a positive integer"),
    ...
    for value in [-1.0, 257.0] {
      ...
      assert!(
        error
          .reason
          .contains("`maxBlockingTasks` must be a positive integer"),
        "unexpected validation error: {}",
        error.reason
      );
    }
  ...
  fn runtime_metrics_preserve_safe_javascript_integer_range() {
    ...
    assert_eq!(
      safe_js_number(beyond_u32).to_bits(),
      BEYOND_U32_JS_NUMBER.to_bits()
    );
    assert_eq!(
      safe_js_number(MAX_SAFE_JS_INTEGER).to_bits(),
  ...
  fn relay_ids_fail_closed_without_reuse_at_u32_exhaustion() {
    ...
    allocator
      .next
      .store(u64::from(u32::MAX), std::sync::atomic::Ordering::Relaxed);
  ...
  fn relay_drop_guard_cleans_only_its_matching_pending_relay() {
    ...
    let replacement_relay_id = state
      .relay_ids
      .reserve()
      .expect("the replacement relay id must be available");
    assert_ne!(
  ...
  fn relay_native_cancellation_settles_before_fallible_host_cancellation() {
    ...
    state
      .panic_on_cancel
      .store(true, std::sync::atomic::Ordering::SeqCst);
    let registration = register_pending_host_timer(
      &state.pending,
      &state.relay_ids,
      1,
  ...
  fn host_timer_cleanup_contains_panicking_payload_drop_across_ffi() {
    const CHILD_ENV: &str = "NAPI_RUNTIME_TEST_HOST_TIMER_PANIC_PAYLOAD_CHILD";
    ...
    let output = std::process::Command::new(std::env::current_exe().unwrap())
      .arg("--exact")
      .arg("adapter::tests::host_timer_cleanup_contains_panicking_payload_drop_across_ffi")
      .arg("--nocapture")
      .env(CHILD_ENV, "1")
      .output()
    ...
    impl Drop for LockCheckingDrop {
      fn drop(&mut self) {
        let pending = self
          .pending
          .upgrade()
          .expect("the pending registry must outlive its waker");
  ...
  fn host_driver_installation_allows_reentrant_eviction_before_publication() {
    ...
    let installed = install_host_driver_registration(
      ...
      || {
        ...
        assert!(
          removed.is_none(),
          "reentrant eviction must see no half-published registration"
        );
        17_u64
      },
      || {
      ...
      |id| {
        ...
        let guard = registration
          .try_lock()
          .expect("rollback must run outside the registration mutex");
        drop(guard);
    ...
    assert!(
      !installed,
      "reentrant eviction must reject the exported registration"
    );
  ...
  #[test]
  fn shared_runtime_rejects_after_shutdown_and_accepts_after_restart() {
    use std::{
      sync::{
        Arc,
    ...
    napi::bindgen_prelude::AsyncRuntime::start(&SharedAsyncRuntime)
      .expect("the adapter runtime must start");
    crate::async_runtime::reset_metrics();
    ...
    napi::bindgen_prelude::AsyncRuntime::spawn_blocking(
      ...
      Box::new(move || {
        first_tx
          .send(())
          .expect("test receiver must still be listening");
      }),
    )
    ...
    napi::bindgen_prelude::AsyncRuntime::shutdown(&SharedAsyncRuntime)
    ...
    let block_on_error =
      napi::bindgen_prelude::AsyncRuntime::block_on(&SharedAsyncRuntime, block_on_future.as_mut())
    ...
    assert!(
      !block_on_ran.load(Ordering::SeqCst),
      "rejected block_on must not poll its future"
    );
    ...
    napi::bindgen_prelude::AsyncRuntime::spawn_blocking(&SharedAsyncRuntime, rejected)
    ...
    assert!(
      ...
      "rejected work must be returned intact"
    );
    assert!(
      crate::async_runtime::metrics().blocking_tasks_started >= 2,
  ...
  fn native_task_host_null_env_retires_payload_and_recovers_exact_delivery() {
    const CHILD_ENV: &str = "NAPI_RUNTIME_TEST_NATIVE_TASK_HOST_NULL_ENV_CHILD";
    ...
    if std::env::var_os(CHILD_ENV).is_some() {
      ...
      spawn_detached(async move {
        completed_tx
          .send(())
          .expect("the completion observer must remain live");
      });
      ...
      let first_payload = Box::into_raw(Box::new(
        NativeCurrentThreadTaskHostPayload::with_drop_observer(
          first_delivery,
      ...
      let replacement_payload = Box::into_raw(Box::new(
        NativeCurrentThreadTaskHostPayload::with_drop_observer(
      ...
      .cast();
      let fake_env = std::ptr::NonNull::<std::ffi::c_void>::dangling()
        .as_ptr()
        .cast();
      unsafe {
        call_native_current_thread_task_host(
    ...
    let output = std::process::Command::new(std::env::current_exe().unwrap())
      .arg("--exact")
      .arg("adapter::tests::native_task_host_null_env_retires_payload_and_recovers_exact_delivery")
      .arg("--nocapture")
      .env(CHILD_ENV, "1")
      .output()
  ...
  fn native_task_host_callback_lease_covers_ack_payload_and_restart() {
    const CHILD_ENV: &str = "NAPI_RUNTIME_TEST_NATIVE_TASK_HOST_CALLBACK_LEASE_CHILD";
    ...
    if std::env::var_os(CHILD_ENV).is_some() {
      ...
      spawn_detached(async move {
        task_completed_tx
          .send(())
          .expect("the task completion observer must remain live");
      });
    ...
    let output = std::process::Command::new(std::env::current_exe().unwrap())
      .arg("--exact")
      .arg("adapter::tests::native_task_host_callback_lease_covers_ack_payload_and_restart")
      .arg("--nocapture")
      .env(CHILD_ENV, "1")
      .output()
  ...
  fn timer_relay_terminal_drop_at_host_turn_budget_settles_without_later_work() {
    const CHILD_ENV: &str = "NAPI_RUNTIME_TEST_TIMER_RELAY_TERMINAL_DROP_CHILD";
    ...
    if let Some(relay_state) = std::env::var_os(CHILD_ENV) {
      ...
      let task_result = timer_task
        .now_or_never()
        .expect("the original timer operation must settle immediately");
      assert_eq!(
        task_result
    ...
    for relay_state in ["awaiting", "complete"] {
      let output = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg(
          "adapter::tests::timer_relay_terminal_drop_at_host_turn_budget_settles_without_later_work",
        )
        .arg("--nocapture")
  ...
  fn relay_eviction_is_decided_by_status_and_probe_never_by_message() {
    ...
    assert!(
      should_evict_for_relay_error(Status::Closing, true),
      "Closing overrides a live probe"
    );
    ...
    assert!(should_evict_for_relay_error(
  ...
  fn cancellation_failures_share_the_three_strike_budget_without_cleanup_double_counting() {
    ...
    assert_eq!(
      cleanup_health.cancellation_accounting(),
      RelayCancellationAccounting::CleanupOnly
    );
    assert_eq!(
  ...
  fn resolve_thread_count_handles_env_inputs() {
    ...
    // A `0` typo must be treated as unset, never fed to a constructor that
    // rejects 0 (`RuntimeOptions` validation), which would panic the host's
    // module_init `expect`.
    assert_eq!(
      resolve_thread_count(Some("0".to_string()), DEFAULT, MAXIMUM),
      DEFAULT
    );
    // Valid positive numbers pass through unchanged.
    assert_eq!(
      resolve_thread_count(Some("4".to_string()), DEFAULT, MAXIMUM),
      4
    );
    // Valid but unsafe values clamp to the construction limit.
    assert_eq!(
      resolve_thread_count(Some("1000".to_string()), DEFAULT, MAXIMUM),
      MAXIMUM
    );
    assert_eq!(
      resolve_thread_count(Some(usize::MAX.to_string()), DEFAULT, MAXIMUM),
      MAXIMUM
    );
    ...
    assert_eq!(
      resolve_thread_count(Some("abc".to_string()), DEFAULT, MAXIMUM),
      DEFAULT
    );
