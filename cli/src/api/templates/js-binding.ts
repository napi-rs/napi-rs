export function createCjsBinding(
  localName: string,
  pkgName: string,
  idents: string[],
  packageVersion?: string,
): string {
  return `${bindingHeader}
${createCommonBinding(localName, pkgName, packageVersion)}
${createLoader(packageVersion)}
module.exports = nativeBinding
${idents
  .map((ident) => `module.exports.${ident} = nativeBinding.${ident}`)
  .join('\n')}
`
}

export function createEsmBinding(
  localName: string,
  pkgName: string,
  idents: string[],
  packageVersion?: string,
): string {
  return `${bindingHeader}
import { createRequire } from 'module'
const require = createRequire(import.meta.url)
const __dirname = new URL('.', import.meta.url).pathname

${createCommonBinding(localName, pkgName, packageVersion)}
${createLoader(packageVersion)}
const { ${idents.join(', ')} } = nativeBinding
${idents.map((ident) => `export { ${ident} }`).join('\n')}
`
}

function createLoader(targetVersion?: string) {
  return `${
    typeof targetVersion !== 'undefined'
      ? `function versionMismatchCheck(bindingPackageVersion) {
  if (bindingPackageVersion !== "${targetVersion}" && process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
    throw new Error(\`Native binding package version mismatch, expected ${targetVersion} but got \${bindingPackageVersion}. You can reinstall dependencies to fix this issue.\`)
  }
}

`
      : ''
  }function unsupportedArchitectureError(platform) {
  loadErrors.push(new Error(\`Unsupported architecture on \${platform} \${process.arch}\`))
}

function loadPackage(loader) {
  try {
    return loader();
  } catch (e) {
    loadErrors.push(e)
  }
}


function load({ local, installed, version }) {
  return (loadPackage(local) || loadPackage(() => {
    const binding = installed();
    version?.();
    return binding
  }));
}
`
}

const bindingHeader = `// prettier-ignore
/* eslint-disable */
// @ts-nocheck
/* auto-generated by NAPI-RS */
`

function createCommonLoader(
  localName: string,
  pkgName: string,
  tuple: string,
  indentMaxDepth = 8,
  packageVersion?: string,
) {
  const indexMax = ' '.repeat(indentMaxDepth)
  const indentMin = ' '.repeat(indentMaxDepth - 2)
  return packageVersion
    ? `return load({
${indexMax}local: () => require('./${localName}.${tuple}.node'), 
${indexMax}installed: () => require('${pkgName}-${tuple}'), 
${indexMax}version: () => versionMismatchCheck(require('${pkgName}-${tuple}/package.json').version),
${indentMin}});`
    : `return load({
${indexMax}local: () => require('./${localName}.${tuple}.node'), 
${indexMax}installed: () => require('${pkgName}-${tuple}'), 
${indentMin}});`
}

function createCommonBinding(
  localName: string,
  pkgName: string,
  packageVersion?: string,
): string {
  function requireTuple(tuple: string, indentMaxDepth = 8) {
    return createCommonLoader(
      localName,
      pkgName,
      tuple,
      indentMaxDepth,
      packageVersion,
    )
  }

  return `const { readFileSync } = require('fs')
let nativeBinding = null
const loadErrors = []

const isMusl = () => {
  let musl = false
  if (process.platform === 'linux') {
    musl = isMuslFromFilesystem()
    if (musl === null) {
      musl = isMuslFromReport()
    }
    if (musl === null) {
      musl = isMuslFromChildProcess()
    }
  }
  return musl
}

const isFileMusl = (f) => f.includes('libc.musl-') || f.includes('ld-musl-')

const isMuslFromFilesystem = () => {
  try {
    return readFileSync('/usr/bin/ldd', 'utf-8').includes('musl')
  } catch {
    return null
  }
}

const isMuslFromReport = () => {
  let report = null
  if (process.report && typeof process.report.getReport === 'function') {
    process.report.excludeNetwork = true
    report = process.report.getReport()
  }
  if (!report) {
    return null
  }
  if (report.header && report.header.glibcVersionRuntime) {
    return false
  }
  if (Array.isArray(report.sharedObjects)) {
    if (report.sharedObjects.some(isFileMusl)) {
      return true
    }
  }
  return false
}

const isMuslFromChildProcess = () => {
  try {
    return require('child_process').execSync('ldd --version', { encoding: 'utf8' }).includes('musl')
  } catch (e) {
    // If we reach this case, we don't know if the system is musl or not, so is better to just fallback to false
    return false
  }
}

function requireNative() {
  if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    return loadPackage(() => require(process.env.NAPI_RS_NATIVE_LIBRARY_PATH));
  }
  if (process.platform === 'android') {
    if (process.arch === 'arm64') {
      ${requireTuple('android-arm64')}
    } 
    if (process.arch === 'arm') {
      ${requireTuple('android-arm-eabi')}
    }
    return unsupportedArchitectureError("Android");
  }
  if (process.platform === 'win32') {
    if (process.arch === 'x64') {
      if ((process.config && process.config.variables && process.config.variables.shlib_suffix === 'dll.a') || (process.config && process.config.variables && process.config.variables.node_target_type === 'shared_library')) {
        ${requireTuple('win32-x64-gnu', 10)}
      }
      ${requireTuple('win32-x64-msvc')}
    }
    if (process.arch === 'ia32') {
      ${requireTuple('win32-ia32-msvc')}
    }
    if (process.arch === 'arm64') {
      ${requireTuple('win32-arm64-msvc')}
    } 
    return unsupportedArchitectureError("Windows");
  }
  if (process.platform === 'darwin') {
    if (process.arch === 'x64') {
      ${requireTuple('darwin-x64')}
    }
    if (process.arch === 'arm64') {
      ${requireTuple('darwin-arm64')}
    }
    const universal = (() => {
      ${requireTuple('darwin-universal')}
    })();
    if (universal) {
      return universal;
    }
    return unsupportedArchitectureError("macOS");
  } 
  if (process.platform === 'freebsd') {
    if (process.arch === 'x64') {
      ${requireTuple('freebsd-x64')}
    }
    if (process.arch === 'arm64') {
      ${requireTuple('freebsd-arm64')}
    }
    return unsupportedArchitectureError("FreeBSD");
  } 
  if (process.platform === 'linux') {
    if (process.arch === 'x64') {
      if (isMusl()) {
        ${requireTuple('linux-x64-musl', 10)}
      }
      ${requireTuple('linux-x64-gnu')}
    } 
    if (process.arch === 'arm64') {
      if (isMusl()) {
        ${requireTuple('linux-arm64-musl', 10)}
      }
      ${requireTuple('linux-arm64-gnu')}
    }
    if (process.arch === 'arm') {
      if (isMusl()) {
        ${requireTuple('linux-arm-musleabihf', 10)}
      }
      ${requireTuple('linux-arm-gnueabihf')}
    }
    if (process.arch === 'loong64') {
      if (isMusl()) {
        ${requireTuple('linux-loong64-musl', 10)}
      }
      ${requireTuple('linux-loong64-gnu')}
    }
    if (process.arch === 'riscv64') {
      if (isMusl()) {
        ${requireTuple('linux-riscv64-musl', 10)}
      }
      ${requireTuple('linux-riscv64-gnu')}
    }
    if (process.arch === 'ppc64') {
      ${requireTuple('linux-ppc64-gnu')}
    }
    if (process.arch === 's390x') {
      ${requireTuple('linux-s390x-gnu')}
    }
    return unsupportedArchitectureError("Linux");
  } 
  if (process.platform === 'openharmony') {
    if (process.arch === 'arm64') {
      ${requireTuple('openharmony-arm64')}
    }
    if (process.arch === 'x64') {
      ${requireTuple('openharmony-x64')}
    }
    if (process.arch === 'arm') {
      ${requireTuple('openharmony-arm')}
    }
    return unsupportedArchitectureError("OpenHarmony");
  }
  loadErrors.push(new Error(\`Unsupported OS: \${process.platform}, architecture: \${process.arch}\`))
}

nativeBinding = requireNative()

// NAPI_RS_FORCE_WASI is a tri-state flag:
//   unset / any other value → native binding preferred, WASI is only a fallback
//   'true'                   → force WASI fallback even if native loaded
//   'error'                  → force WASI and throw if no WASI binding is found
// Treating any non-empty string as truthy (the historical behavior) meant
// NAPI_RS_FORCE_WASI=false, NAPI_RS_FORCE_WASI=0, etc. inadvertently triggered
// the WASI path, causing ENOENT for packages shipped without a .wasi.cjs file.
const forceWasi =
  process.env.NAPI_RS_FORCE_WASI === 'true' || process.env.NAPI_RS_FORCE_WASI === 'error'

if (!nativeBinding || forceWasi) {
  let wasiBinding = null
  let wasiBindingError = null
  try {
    wasiBinding = require('./${localName}.wasi.cjs')
    nativeBinding = wasiBinding
  } catch (err) {
    if (forceWasi) {
      wasiBindingError = err
    }
  }
  if (!nativeBinding || forceWasi) {
    try {
      wasiBinding = require('${pkgName}-wasm32-wasi')
      nativeBinding = wasiBinding
    } catch (err) {
      if (forceWasi) {
        if (!wasiBindingError) {
          wasiBindingError = err
        } else {
          wasiBindingError.cause = err
        }
        loadErrors.push(err)
      }
    }
  }
  if (process.env.NAPI_RS_FORCE_WASI === 'error' && !wasiBinding) {
    const error = new Error('WASI binding not found and NAPI_RS_FORCE_WASI is set to error')
    error.cause = wasiBindingError
    throw error
  }
}

if (!nativeBinding) {
  if (loadErrors.length > 0) {
    const error = new Error(
      \`Cannot find native binding. \` +
        \`npm has a bug related to optional dependencies (https://github.com/npm/cli/issues/4828). \` +
        'Please try \`npm i\` again after removing both package-lock.json and node_modules directory.',
    )
    // assign instead of the \`new Error(message, { cause })\` options form,
    // which Node < 16.9 silently ignores
    error.cause = loadErrors.reduce((err, cur) => {
      cur.cause = err
      return cur
    })
    throw error
  }
  throw new Error(\`Failed to load native binding\`)
}
`
}
