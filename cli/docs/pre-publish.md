...
| rootPublisher       | --root-publisher          | 'npm' \| 'pnpm' \| 'yarn' \| 'yarn-classic' | false    |                | The package manager that publishes the root package. pnpm and yarn berry replace `exports` with `publishConfig.exports` while packing, so for them the root package validation targets the publish-effective export map. Defaults to the `napi.rootPublisher` config field, and validates both export maps when neither is set |
