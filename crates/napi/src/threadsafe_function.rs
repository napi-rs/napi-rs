...
use crate::{
  ...
  check_status, extract_error_cause, get_error_message_and_stack_trace, sys, Env, Error, JsError, Result, Status,
};
        ...
        let callback_arg = if status == sys::Status::napi_pending_exception {
          ...
          get_error_message_and_stack_trace(raw_env, exception).and_then(|reason| {
            Err(Error {
              ...
              cause: extract_error_cause(unsafe { Unknown::from_raw_unchecked(raw_env, exception) }).unwrap_or(None),
