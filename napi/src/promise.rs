...
unsafe extern "C" fn call_js_cb<T, V: NapiValue>(
  ...
  match js_value_to_resolve {
    Ok(v) => {
      let open_handle_status = sys::napi_open_handle_scope(raw_env, &mut handle_scope);
      debug_assert!(
  ...
  let close_handle_scope_status = sys::napi_close_handle_scope(raw_env, handle_scope);
  debug_assert!(
