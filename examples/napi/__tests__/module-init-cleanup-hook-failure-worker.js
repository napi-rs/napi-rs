...
if (workerData.role === 'failure') {
  const failedModule = new Module(
    `${workerData.addonPath}:cleanup-hook-failure`,
  )
...
} else if (workerData.role === 'recovery') {
  ...
  assert.equal(
    await recoveredModule.exports.moduleInitRollbackAsyncProbe(41),
    42,
  )
  parentPort.postMessage({ type: 'ready' })
} else {
  throw new TypeError(`unknown cleanup-hook worker role: ${workerData.role}`)
}
