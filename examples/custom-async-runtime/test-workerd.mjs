...
await build({
  ...
  external: ['*.wasm'],
  // Rebind `setImmediate` to the global scope: workerd timer functions throw
  // "Illegal invocation" when called with a different receiver, and emnapi
  // calls a captured reference through its feature object.
  inject: ['workerd/globals-shim.mjs'],
})
