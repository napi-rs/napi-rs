  ...
  case 'linux':
    switch (arch) {
      ...
      case 'arm':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, '${localName}.linux-arm-musleabihf.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./${localName}.linux-arm-musleabihf.node')
            } else {
              nativeBinding = require('${pkgName}-linux-arm-musleabihf')
            }
          } catch (e) {
            loadError = e
          }
