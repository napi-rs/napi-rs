export function createCjsBinding(localName: string, pkgName: string): string {
...
export function createEsmBinding(cjsName: string, idents: string[]): string {
  return `
  import { createRequire } from "module";
  const require = createRequire(import.meta.url);
  
  const binding = require("./${cjsName}.cjs");
  
  ${idents
    .map((ident) => `export const ${ident} = binding.${ident}`)
    .join('\n  ')}
`
}
