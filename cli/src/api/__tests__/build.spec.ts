...
test('a header that cannot spell the binding target skips the checker', (t) => {
  ...
  // asked: one that contains neither the name nor a `\u` escape that could
  // become part of one cannot bind it, and the parse alone settles the
  // question. `checked` reports whether that shortcut applied, so this holds
  // it in place by observation rather than by timing anything.
  ...
  // an escape with nothing to do with the name gives up the shortcut too:
  // whether one could have built the name is exactly the question the parse
  // cannot answer, so it is not guessed at here
  const unrelated = scan("export declare const nam\\u0065: 'native'")
  t.true(unrelated.checked)
  t.false(unrelated.ownsName)
})
