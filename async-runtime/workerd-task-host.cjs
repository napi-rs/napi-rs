...
/**
 * Register and return an exact disposer for one native CurrentThread task
 * host on a managed workerd binding instance.
 */
function registerWorkerdCurrentThreadTaskHost(binding) {
  ...
  const registerCurrentThreadTaskHost = Reflect.get(
    binding,
  ...
  const unregisterCurrentThreadTaskHost = Reflect.get(
    binding,
